...
Codex Lab is a desktop-only Obsidian plugin that wraps your authenticated Codex CLI session. It is built for users who want an ObsidianCode-like agent sidebar, but powered by Codex instead of Claude Code.

Core chat and visual generation are routed through Codex CLI. Codex Lab does **not** require an OpenAI API key for normal use.

## Current Release

Latest BRAT release: **0.3.0**
...
Codex Lab can generate and embed PNG or SVG visual assets from the current note.
...
1. Open a note.
2. Click the image button in the Codex Lab sidebar.
3. Choose `PNG via Codex image generation` or `SVG via Codex code generation`.
4. Choose a visual format.
5. Codex Lab analyzes the note and drafts a production-ready image prompt.
6. Review or edit the prompt.
7. Codex Lab generates the asset and embeds it into the note.
...
Open Obsidian Settings → Community plugins → Codex Lab.

Codex Lab settings include:
...
- **macOS**: Codex Lab auto-detects Homebrew, npm, and NVM Codex paths such as `/opt/homebrew/bin/codex`.
- **Windows**: Codex Lab auto-detects npm paths such as `%APPDATA%\npm\codex.cmd`. When this `.cmd` wrapper causes Node spawn issues, Codex Lab runs the underlying `@openai/codex/bin/codex.js` entrypoint through `node`.
...
Codex Lab supports BRAT installation from GitHub releases.
...
5. Enable **Codex Lab** in Obsidian Community Plugins.
...
Codex Lab is designed to make Obsidian notes feel native inside Codex CLI:

- Open a note and Codex Lab detects it automatically.
...
- Run `Codex Lab: Attach current note to chat` from Obsidian commands or bind it to a hotkey.
- Ask Codex normally; Codex Lab sends the selected note context through Codex CLI.
...
4. Codex Lab recommends related notes with clear reasons.
5. Add useful notes to Codex Lab context with `+`.
...
- `Codex Lab: Build Memory Map`
- `Codex Lab: Find related notes for current note`
...
- On Windows, `Codex CLI path` may point to `codex.cmd`; Codex Lab will internally prefer the matching `codex.js` entrypoint when available.
- On macOS/Linux, Codex Lab keeps the normal executable path and does not use the Windows `.cmd` workaround.
...
```text
<vault>/.obsidian/plugins/codex-lab/
  main.js
  manifest.json
  styles.css
```

Then enable **Codex Lab** in Obsidian settings.
...
Codex Lab is in active MVP development. Current focus areas:
